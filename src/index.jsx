...
store.subscribe(()=>{
    ...
    /**
     * When the products array is populated, that means the saga round trip has completed,
     * and the application can be rendered.
     * Rendering before the products arrived would result in the server-generated content being replaced with
     * a blank page.
     */
...
const fetchDataForLocation = location=>{
    ...
    /**
     * If the location is the details route, fetch details for one product
     */
    if (location.pathname.includes(`product`)) {
        store.dispatch({type:`REQUEST_FETCH_PRODUCT`, code:location.pathname.split('/')[2]});
    }
    
    /**
     * If the location is the standard route, fetch an undetailed list of all products
     */

    if (location.pathname === "/" ) {
        store.dispatch({type: `REQUEST_FETCH_PRODUCTS`});
    }
